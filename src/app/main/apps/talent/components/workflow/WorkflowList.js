...
import WorkflowAddListItem from './WorkflowAddListItem';
...
function WorkflowList(props) {
  ...
  return (
    <div className={props.className}>
        ...
        <Accordion expanded={expanded === ('panel' + idx )} onChange={handleChange(('panel' + idx))}>
          <AccordionSummary aria-controls="panel1d-content" id="panel1d-header" >
            <div className="flex flex-row justify-between w-full">
              <Typography className="flex items-center">{expanded === ('panel' + idx ) ?'':(item.name)}</Typography>
              <IconButton
      ...
      {!expanded &&
      <WorkflowAddListItem onListItemAdd={handleListItemAdd}/>
      }
    </div>
  );
}

...
function TodoDialog(props) {
  ...
  const todoDialog = useSelector(({ jobSearch }) => jobSearch.jobs.jobDialog);
  ...
  const initDialog = useCallback(() => {
     ...
     * Dialog type: 'edit'
     */
    if (jobDialog.type === 'edit' && todoDialog.data) {
  ...
  return (
    <Dialog {...jobDialog.props} onClose={closeTodoDialog} fullWidth maxWidth="sm">
      <AppBar position="static" elevation={0}>
        <Toolbar className="flex w-full">
          <Typography variant="subtitle1" color="inherit">
            {jobDialog.type === 'new' ? 'New Todo' : 'Edit Todo'}
          </Typography>
        </Toolbar>
      </AppBar>

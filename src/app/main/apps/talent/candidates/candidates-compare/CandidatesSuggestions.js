...
import { getSuggestionCandidates, getCandidates, setCompare } from './store/candidatesCompareSlice';
...
function CandidatesSuggestions(props) {
	...
	const filterSidebarOpen = useSelector(({ candidatesCompare }) => candidatesCompare.sidebars.filterSidebarOpen);
	...
	return (
		<div className="flex flex-col flex-auto flex-shrink-0">
			...
			>
				<CandidatesSidebarContent />
			</Drawer>
			<FuseAnimateGroup enter={{ animation: 'transition.slideUpBigIn' }} 
			className="flex flex-wrap m-5">
				{suggestions.slice((leftPagination.page * leftPagination.size), (leftPagination.page * leftPagination.size + leftPagination.size)).map((user) =>
					...
					>
						<Card elevation={1} className={((selected && selected.includes(parseInt(user.id))) ? classes.cardBorder : "") + " flex flex-col rounded-8 border-blue-900 "}
							onClick={() => setSelectedSuggestion(user.id)}>
							<CardHeader

...
export const getCompanyQuestionTemplates = createAsyncThunk(
  'company/templates/questions',
  async (all, { getState }) => {
    let user = getState().auth.user.data;
    const response = await axios.get(`http://accessed-job-service.us-west-2.elasticbeanstalk.com/api/talent/company/${user.preferredCompany}/questions/templates?all=${all}`, {headers: {userId: user.id}});
...
export const { selectAll: selectRoles, selectById: selectRolesById } =
  questionTemplatesAdapter.getSelectors((state) => state.rolesApp.roles);


const questionTemplatesSlice = createSlice({
  name: 'company/roles',
  initialState: questionTemplatesAdapter.getInitialState({
    searchText: '',
    data: [],
    selectedRoleIds: [],
    roleDialog: {
  ...
  extraReducers: {
    [updateRole.fulfilled]: questionTemplatesAdapter.upsertOne,
    [deleteRole.fulfilled]: questionTemplatesAdapter.removeOne,
    [getCompanyQuestionTemplates.fulfilled]: (state, action) => {
      const { payload, routeParams } = action;
      questionTemplatesAdapter.setAll(state, action.payload);
...
export const {
  ...
  closeEditRoleDialog,
  setSearchText,
  setFilter } = questionTemplatesSlice.actions;

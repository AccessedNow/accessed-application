...
function CandidateToolbar(props) {
  ...
  function handleCheckChange(event) {
    return event.target.checked ? dispatch(selectAllCandidates()) : dispatch(deselectAllCandidates());
  }
  ...
  return (
    <div className="flex flex-1 items-center sm:px-8">
      <Checkbox
        ...
        checked={selectedCandidateIds.length === Object.keys(candidates).length && selectedCandidateIds.length > 0}
        indeterminate={
          selectedCandidateIds.length !== Object.keys(candidates).length && selectedCandidateIds.length > 0
        }
      />

...
import { updatePagination } from '../store/candidatesSlice';
...
const EnhancedTable = ({ columns, data, onRowClick }) => {
	...
	let pagination = useSelector(({ jobApp }) => jobApp.candidates.pagination);
	...
	const handleChangePage = (event, newPage) => {
		...
		dispatch(updatePagination(pageData));
	};
	...
	return (
		<div className="flex flex-col min-h-full sm:border-1 overflow-hidden">
						...
						{headerGroups.map(headerGroup => (
							<TableRow {...headerGroup.getHeaderGroupProps()}>
								{headerGroup.headers.map(column => (
									// (column.render('Header') === "Rate" && visibleColumns.find(m => m.name === "Evaluation" && m.checked !== true))
									// ?
									<TableCell
			...
			<TablePagination
				...
				ActionsComponent={CandidatesTablePaginationActions}
			/>
		</div>
	);
};

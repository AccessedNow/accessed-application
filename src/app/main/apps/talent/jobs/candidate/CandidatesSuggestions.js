...
function CandidatesSuggestions(props) {
	...
	const leftPagination = useSelector(({ jobApp }) => jobApp.candidates.pagination);
	 const selected = useSelector(state => state.jobApp.candidates.selected);
		...
		<div className="flex flex-col flex-auto flex-shrink-0 w-full">
			<FuseAnimateGroup enter={{ animation: 'transition.slideUpBigIn' }} className="flex flex-wrap">
				{candidates && candidates.slice((leftPagination.page * leftPagination.size), (leftPagination.page * leftPagination.size + leftPagination.size)).map((user) =>
					...
					>
						<Card elevation={1} className={((selected && parseInt(selected.id)===parseInt(user.id)) ? classes.cardBorder : "") + " flex flex-col rounded-8 border-blue-900 "}
							onClick={() => {
								dispatch(setSelectedItem(user.id));
								history.push('/apps/candidates/' + user.id);
...
const ThreeDotsMenu = (props) => {
	...
	const dispatch = useDispatch();
	const compare = useSelector(({ jobApp }) => jobApp.candidates.compare);
	const selected = useSelector(({ jobApp }) => jobApp.candidates.selected);

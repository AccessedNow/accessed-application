...
function CandidatesSuggestionHeader(props) {
	...
	const pagination = useSelector(({ jobApp }) => jobApp.candidates.pagination);
	...
	return (
		<div className="flex flex-1 items-center justify-between">
			...
			<div className="flex flex-1 items-center justify-end">
				...
				<span><Typography>{pagination.page + 1} / {candidates && candidates.length / pagination.size}</Typography></span>
				<IconButton
					color="primary"
					onClick={nextPage}
				>
...
const ThreeDotsMenu = (props) => {
	...
	const dispatch = useDispatch();
	const compare = useSelector(({ jobApp }) => jobApp.candidates.compare);
	const selected = useSelector(({ jobApp }) => jobApp.candidateSuggestions.selected);

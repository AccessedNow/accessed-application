...
export const getCompanyEmailTemplates = createAsyncThunk(
  'company/templates/emails',
  async (all, { getState }) => {
...
export const { selectAll: selectRoles, selectById: selectRolesById } =
  emailTemplatesAdapter.getSelectors((state) => state.rolesApp.roles);


const emailTemplatesSlice = createSlice({
  name: 'company/roles',
  initialState: emailTemplatesAdapter.getInitialState({
    searchText: '',
    data: [],
    selectedRoleIds: [],
    roleDialog: {
  ...
  extraReducers: {
    [updateRole.fulfilled]: emailTemplatesAdapter.upsertOne,
    [deleteRole.fulfilled]: emailTemplatesAdapter.removeOne,
    [getCompanyEmailTemplates.fulfilled]: (state, action) => {
      const { payload, routeParams } = action;
      emailTemplatesAdapter.setAll(state, action.payload);
...
export const {
  ...
  closeEditRoleDialog,
  setSearchText,
  setFilter } = emailTemplatesSlice.actions;
